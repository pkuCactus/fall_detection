...
class TestReadImagesetSplit:
    ...
    def test_image_names_with_spaces(self, tmp_path):
        ...
        (imagesets_dir / "train.txt").write_text("scene 001 frame 01\nimage with spaces\nregular_name\n")
...
class TestIntegration:
    ...
    def test_yoloworld_style_mapping(self, sample_voc_dataset, tmp_path):
        """Test YOLOWorld style mapping where all classes map to one."""
        config = {
            "datasets": {
                "train_dirs": [str(sample_voc_dataset)],
            },
