...
class AnchorDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=None, version="v5", *args, inplace=True, **kwargs):
        """Initialize AnchorDetect head.

        Args:
            nc: number of classes
            anchors: list of anchor definitions
            version: YOLO get box decoding version ("v3" or "v5")
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if self.training:
                continue  # training, return raw features for loss computation
