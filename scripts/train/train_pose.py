"""Train pose estimator with YOLOv8-pose (config-based).
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train pose estimator with YOLOv8-pose"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override config values, e.g., 'epochs=100,batch=32'",
    )
...
def main():
    ...
    data_yaml = cfg.get("data", "data/fall_pose.yaml")
    epochs = cfg.get("epochs", 100)
    imgsz = cfg.get("imgsz", 128)
    batch = cfg.get("batch", 64)
    model_name = cfg.get("model", "yolov8n-pose.pt")
    project = cfg.get("project", "yolov8-pose")
    ...
    print(f"Starting YOLOv8-pose training...")
    ...
    print(f"  Output: runs/pose/{project}/{name}/")
    ...
    best_path = os.path.join("runs/pose", project, name, "weights", "best.pt")
    out_path = os.path.join("runs/pose", project, name, "best.pt")
    ...
    print(f"\nTraining complete. Results saved to: runs/pose/{project}/{name}/")
