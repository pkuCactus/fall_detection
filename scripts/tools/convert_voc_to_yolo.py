...
def parse_voc_xml(
    ...
    if size is not None:
        ...
        width = (
            int(width_elem.text) if width_elem is not None and width_elem.text else 0
        )
        height = (
    ...
    if width <= 0 or height <= 0:
        if image_path and image_path.exists():
            ...
            raise ValueError(
                f"Invalid size {width}x{height} in {xml_path} and no image available"
            )
...
def convert_dataset_split(
    ...
    for data_dir in data_dirs:
        ...
        if use_imagesets:
            ...
            if image_ids is None:
                print(
                    f"    Warning: ImageSets/{split_name}.txt not found, using all XMLs"
                )
            ...
            try:
                ...
                for box in boxes:
                    ...
                    if voc_class in voc_to_yolo_name:
                        ...
                        if yolo_class in yolo_name_to_id:
                            ...
                            print(
                                f"    Warning: YOLO class '{yolo_class}' not in names list"
                            )
                    else:
                        print(
                            f"    Warning: VOC class '{voc_class}' not in class_mapping"
                        )
                ...
                if not valid_boxes:
                    ...
                    if len(boxes) == 0:
                        ...
                        print(
                            f"    Note: {xml_file.name} has {len(boxes)} objects but none in class_mapping"
                        )
                ...
                with open(label_file, "w") as f:
                    for box in valid_boxes:
                        ...
                        f.write(
                            f"{class_id} {box['x_center']:.6f} {box['y_center']:.6f} "
    ...
    if no_label_count > 0:
        print(
            f"  {split_name}: Converted {converted}, Skipped {skipped}, No labels: {no_label_count} (copied to no_labels/)"
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "--output-dir", "-o", type=Path, help="Override output directory from config"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Print configuration without converting"
    )
    ...
    for split_name, (conv, skip, no_label) in results.items():
        if no_label > 0:
            print(
                f"  {split_name}: Converted {conv}, Skipped {skip}, No labels: {no_label}"
            )
        else:
    ...
    if total_no_label > 0:
        print(
            f"  Total: Converted {total_conv}, Skipped {total_skip}, No labels: {total_no_label}"
        )
        print(
