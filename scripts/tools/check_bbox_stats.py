...
示例:
  ...
  python check_bbox_stats.py --format voc --data-dir data/VOC2007,data/VOC2012 --split train
    ...
    parser.add_argument("--data-dir", type=str,
                        help="VOC 数据目录，多个目录用逗号分隔")
    parser.add_argument("--split", type=str, default="train",
                        help="VOC 数据划分: train/val/test (默认: train)")

    # 通用参数
    parser.add_argument("--min-size", type=int, default=0,
                        help="过滤小于此尺寸的 bbox (默认: 0)")
    parser.add_argument("--max-size", type=int, default=0,
                        help="过滤大于此尺寸的 bbox (默认: 0, 表示不限制)")
...
def load_coco_bboxes(json_path: str) -> Tuple[List[Dict], Dict[int, Dict]]:
    """加载 COCO 格式的 bbox.
...
def find_extreme_bboxes(bboxes: List[Dict], widths: np.ndarray, heights: np.ndarray,
                        n: int = 5) -> str:
    """找出极端尺寸的 bbox."""
    areas = widths * heights
    ...
    for i, idx in enumerate(smallest_idx, 1):
        bbox = bboxes[idx]
        lines.append(f"    {i}. {bbox['w']:.0f}x{bbox['h']:.0f} (area={widths[idx]*heights[idx]:.0f}) - {bbox.get('image_file', bbox.get('img_path', 'unknown'))}")
...
def main():
    ...
    if args.format == "coco":
        ...
        bboxes, images = load_coco_bboxes(args.json)
