...
**核心约束**: 端到端 latency ~34ms，纯视觉pipeline
...
**设计哲学**: Pipeline A 走"精细化多模型融合"路线；Pipeline B 走"端到端开放词汇轻量"路线。

---

## Slide 4: Pipeline A 总流程

### `FallDetectionPipeline` — 端到端数据流
...
[process_frame]
    ...
    │
    ▼
输出: tracks, kpts, scores, falling_flags, alarms
```
...
## Slide 5: Pipeline A — 检测层 (PersonDetector)
...
## Slide 6: Pipeline A — 跟踪层 (ByteTrackLite)

### `src/fall_detection/core/tracker.py`

**设计目标**: 裁剪版 ByteTrack，仅用 IoU + Kalman，无 ReID，极致轻量。

```
输入: List[Detection{bbox, conf}]
    │
    ▼
...
Step 3: 第一次匹配 ── confirmed/tentative tracks ↔ dets_high
    │   代价矩阵: 1 - IoU
    │   匈牙利算法 (linear_sum_assignment)
    │   阈值: match_thresh (默认 0.8，即 IoU >= 0.2)
    │
    ▼
Step 4: 第二次匹配 ── 未匹配 tracks ↔ dets_low
    │   (低分框可能是遮挡物体，帮助找回丢失轨迹)
    │
    ▼
...
Step 6: 未匹配高分检测框 → 新建 tentative Track
    ...
    │
    ▼
输出: 当前 active tracks (time_since_update == 0)
```
...
## Slide 7: Pipeline A — 姿态估计 (PoseEstimator)

### `src/fall_detection/core/pose_estimator.py`
...
## Slide 8: Pipeline A — 分类器设计

### 双分类器支持 (`classifier.py` / `simple_classifier.py`)
...
输入 2: 17 关键点 (17, 3) → flatten 51-d
    ...
    ▼
输入 3: 8-d 运动特征
    │    [vx, vy, ax, ay, w, h, h_ratio, n_ground]
...
Backbone:
  ...
  ResBlock(64→64) → ReLU
  BasicBlock(64→64) → ReLU
  ResBlock(64→128, s=2) → ReLU
  BasicBlock(128→128) → AvgPool → Flatten → 128-d
...
## Slide 9: Pipeline A — 规则引擎 (RuleEngine)

### `src/fall_detection/core/rules.py`
...
| 规则 | 名称 | 触发条件 | 设计意图 |
|------|------|----------|----------|
| **A** | 高度压缩比 | `h_ratio` (头-踝距离 / bbox高) 低 + 地面接触点 ≥ 2 | 核心跌倒特征：身体压缩贴地 |
| **B** | 地面区域 | 若配置 `ground_roi`，检查 bbox 底部是否进入地面区域；否则 fallback 检查底边贴近 bbox 底部 | 防止空中误判，确认人已落地 |
| **C** | 动→静转换 | 早期位移 `avg_early > motion_thresh` 且 晚期 `avg_late < static_thresh`；或 lying + 全程静止 | 跌倒后通常停止移动 |
| **D** | 垂直快速下降 | `vy_px_s > fall_vy_thresh` + (`h_ratio` 低 或 `lying`) | 捕捉跌落瞬间 |
| **E** | 向下加速度 | `accel > accel_thresh` + (`h_ratio` 低 或 `lying`) | 加速度辅助确认 |
| **F** | 关键点不可见 | 可见关键点比例 < `no_keypoint_thresh` | 极端姿态/遮挡时兜底 |

#### 姿态分类逻辑 (`_classify_posture`)

```
if kpt_aspect > 1.5 or torso_angle > 55:          → lying  (光轴方向跌倒)
elif bbox_aspect > 1.3 and kpt_aspect > 1.0 and h_ratio < 0.65: → lying
elif h_ratio > 0.75:                               → standing
elif h_ratio > 0.50:
    if hip_ratio < 0.45:  → sitting
    else:                 → standing (if h_ratio>0.65) else sitting
elif h_ratio > 0.35:
    if hip_ratio < 0.45:  → sitting   (减少坐姿误判)
    else:                 → crouching
else:                                               → lying
```
...
## Slide 10: Pipeline A — 融合决策 (FusionDecision)

### `src/fall_detection/core/fusion.py`
...
## Slide 11: Pipeline A — 抽帧优化与缓存策略

### 核心设计: `skip_frames = 2` → 每 3 帧做一次检测
...
## Slide 13: Pipeline B — YOLO-World 开放词汇检测
...
```yaml
# configs/pipeline/yolo_world.yaml
classes:
  - "person standing"
  - "person sitting"
  - "person squatting"
...
```
models/pretrained/        # 预训练权重 (git-lfs 或手动放置)
...
| 场景 | 推荐 Pipeline | 理由 |
|------|--------------|------|
| 已标注数据充分，精度优先 | **A + FallClassifier** | 关键点规则可解释性强，分类器融合精度高 |
